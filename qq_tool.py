...
Bundled inside the user plugin ``~/.hermes/plugins/hermes-napcat``.
...
def _init(http_api: str, access_token: str = "", owners: list[str] | None = None, admins: list[str] | None = None) -> None:
    global _http_api, _access_token, _owners, _admins
...
def _role_for_sender(sender_id: str) -> str:
    ...
    if sender_id and sender_id in _owners:
        return "owner"
    if sender_id and sender_id in _admins:
        return "admin"
    return "user"
...
def _require_admin() -> str | None:
    """Return an error string if the current user is not an admin/owner."""
    sender, role = _current_identity()
    if role not in {"owner", "admin"}:
        return f"此操作需要管理员权限（当前用户 {sender or 'unknown'} 不是管理员）。"
    return None
